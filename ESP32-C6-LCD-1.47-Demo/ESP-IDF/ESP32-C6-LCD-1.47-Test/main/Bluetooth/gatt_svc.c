...
//UUID: 00152215-1212-EFDE-1523-785FEABCD123
//-------------------------------------------------------------------------------------------------------//

//define sensors uuid //UUID: 00000000-0000-0000-0000-000000000001
static const ble_uuid128_t sensor_chr_uuid =
    BLE_UUID128_INIT(0x00, 0x00, 0x00, 0x00, 
                     0x00, 0x00, 0x00, 0x00, 
                     0x00, 0x00, 0x00, 0x00, 
                     0x00, 0x00, 0x00, 0x01);
static uint16_t sensor_chr_val_handle;
static uint16_t sensor_chr_conn_handle = 0;
static bool     sensor_chr_conn_inited = false;
...
static int sensor_chr_access(uint16_t conn_handle, uint16_t attr_handle,
    ...
    case BLE_GATT_ACCESS_OP_READ_CHR:
        ...
        if (attr_handle == sensor_chr_val_handle) {
            ...
            rc = os_mbuf_append(ctxt->om, &sensor_data,
                                sizeof(sensor_data));
...
void send_sensor_notify(void)
{
    if (sensor_ind_status && sensor_chr_conn_inited) {
        uint32_t sensor_data = 0x12345678;
        struct os_mbuf *om = ble_hs_mbuf_from_flat(&sensor_data, sizeof(sensor_data));
        if (!om) {
        ...
        .characteristics =
            (struct ble_gatt_chr_def[]){/* LED characteristic */
                                        {.uuid = &sensor_chr_uuid.u,
                                         ...
                                         .flags = BLE_GATT_CHR_F_WRITE | BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_NOTIFY| BLE_GATT_CHR_F_INDICATE,
...
void gatt_svr_subscribe_cb(struct ble_gap_event *event) {
    ...
    else if( event->subscribe.attr_handle == sensor_chr_val_handle){
